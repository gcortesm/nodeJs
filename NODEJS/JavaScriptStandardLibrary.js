...
function internacionalizacion() {
    //internalizacion de moneda.
    let modena = Intl.NumberFormat("es", { style: "currency", currency: "COP" });
    console.log(modena.format(12345823));

    let data = [0.005, 0.75, 1];
    let formatData = Intl.NumberFormat(undefined, { style: "percent", minimumFractionDigits: 1, maximumFractionDigits: 6 }).format;

    console.log(data.map(formatData));

    let arabic = Intl.NumberFormat("ar", { useGrouping: false }).format;

    console.log(data.map(arabic));
    console.log(arabic(1234567890));

    let indi = Intl.NumberFormat('hi-IN-u-nu-deva', { useGrouping: false }).format;
    console.log(indi(1234567890));

    let date = new Date('2021-03-16T08:54:13Z');
    console.log(date);

    console.log(Intl.DateTimeFormat('en-US', { weekday: 'long', month: 'long', year: 'numeric', day: 'numeric' }).format(date));
    console.log(Intl.DateTimeFormat('fr-FR', { weekday: 'long', month: 'long', year: 'numeric', day: 'numeric' }).format(date));
    console.log(Intl.DateTimeFormat('es-ES', { weekday: 'long', month: 'long', year: 'numeric', day: 'numeric', hour12: true, hourCyrcle: 'h24' }).format(date));
    /**
     * h11: media noche en 0 y 11 de la noche 
     * h12: media noche 12
     * h23: media noche 0 y 11 serian las 23pm
     * h24: media noche 24
     */

    //Deletrear dias de la semana y mes 
    let opts = { hour: 'numeric', minute: '2-digit' };
    console.log(Intl.DateTimeFormat('en-US', opts).format(date));
    //Calendarios no cirstianos
    opts = { weekday: 'long', month: 'long', year: 'numeric', day: 'numeric', };
    console.log(Intl.DateTimeFormat('en-u-ca-hebrew', opts).format(date));

    //opts = {year:"numeric",era:"short"};
    console.log(Intl.DateTimeFormat('en-u-ca-buddhist', opts).format(date));
    console.log(Intl.DateTimeFormat('en-u-ca-chinese', opts).format(date));
...
function UsandoUrl() {
    ...
    url2.pathname = 'api/search';
    url2.search = 'query=test';
    console.log(url2.toString());

    url2.searchParams.append('q', 'valor');
    url2.searchParams.append('v', 'valor');
    console.log(url2.searchParams);

    url2.searchParams.set('y', 'yvalue');
    console.log(url2.searchParams);

    console.log(url2.searchParams.getAll('y'));

    for (let val of [...url2.searchParams]) {
        console.log(val);
        url2.searchParams.delete(val[0]);
    }
 ...
 * son funciones de tipo call back.
 * Funciona todos los navegadores. IE falla a ratos 
 * setTimeOut:
 * setInterval:
 * 
 */
function timers() {
    ...
    setTimeout(() => {
        console.log('Esto se imprime despues de 3 segundo');
    }, 3000);

    setTimeout(() => {
        console.log('Esto se imprime despues de 1 segundo');
    }, 1000);

    setTimeout(() => {
        console.log('Esto se imprime despues de 5 segundo');
    }, 5000);

    //Ahora usando setInterval
